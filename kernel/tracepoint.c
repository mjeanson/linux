					     ...
					     void *data, int prio, bool warn)
{
	...
	tp_func.data = data;
	tp_func.prio = prio;
	ret = tracepoint_add_func(tp, &tp_func, prio, warn);
...
/**
 * tracepoint_probe_register_prio_may_exist_mayfault -  Connect a probe to a tracepoint with priority
 * @tp: tracepoint
 * @probe: probe handler
 * @data: tracepoint data
 * @prio: priority of this function over other registered functions
 ...
 * if the tracepoint is already registered.
 */
int tracepoint_probe_register_prio_may_exist_mayfault(struct tracepoint *tp, void *probe,
					     void *data, int prio)
{
	if (!(tp->flags & TRACEPOINT_MAYFAULT))
		return -EINVAL;
...
EXPORT_SYMBOL_GPL(tracepoint_probe_register_prio_may_exist_mayfault);
				   ...
				   void *data, int prio)
{
	...
	return __tracepoint_probe_register_prio(tp, probe, data, prio, true);
}
